...
Choose **Continue supplied score (experimental)** under **Source Score Use** to let YuE2 extend an uploaded or transcribed score before generating audio. Supply lyrics or an instrumental section plan for the whole intended song. The source notation becomes the planner prefix; continuation does not guarantee a target duration. Maximum Song Duration remains an upper limit.
...
`custom_settings.extend_score`: 0 uses the supplied/transcribed score as complete (default); 1 continues it with the ABC planner before audio generation. Requires source audio or ABC and mode 0/1/3; ordinary YuE2 only. Experimental; duration remains an upper limit.
